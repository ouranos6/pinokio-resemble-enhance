...
class Solver:
    ...
    @staticmethod
    @cache
    def _solve_exponential_decay_a(n: int) -> float:
        if n <= 2:
            return 1.0
    ...
    def exponential_decay_mapping(t, n=4):
        ...
        a = Solver._solve_exponential_decay_a(int(n))
        if abs(a - 1.0) < 1e-6:
            return t

        if isinstance(t, torch.Tensor):
            a_t = torch.tensor(a, device=t.device, dtype=t.dtype)
            return (torch.pow(a_t, t) - 1.0) / (a - 1.0)

        if isinstance(t, np.ndarray):
            return (np.power(a, t) - 1.0) / (a - 1.0)
